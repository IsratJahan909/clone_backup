...
class _AttendanceScreenState extends State<AttendanceScreen> {
  // using local sqlite database for attendance
  ...
  Future<void> _loadAttendance() async {
    ...
    try {
      if (!kIsWeb) {
        await AttendanceLocalDb
            .instance
            .database; // ensure db is ready on non-web
      }
      final isAdmin = ApiService.userRole?.toUpperCase() == 'ADMIN';
      final int? employeeId = ApiService.userId;

      List<Map<String, dynamic>> data;
      if (isAdmin) {
        data = await AttendanceLocalDb.instance.getAllRecords();
      } else {
        data = employeeId != null
            ? await AttendanceLocalDb.instance.getRecordsForEmployee(employeeId)
            : [];
      }
  ...
  Future<void> _handleCheckInOut() async {
    ...
    try {
      todaysRecord = _history.firstWhere(
        (r) => r['date'] != null && r['date'].toString().startsWith(today),
      );
    } catch (_) {
      todaysRecord = null;
    }
    ...
    try {
      final int employeeId = ApiService.userId ?? 0;

      if (todaysRecord == null) {
        await AttendanceLocalDb.instance.checkIn(employeeId);
        ...
        await AttendanceLocalDb.instance.checkOut(todaysRecord['id']);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Day already completed.')));
      }
      _loadAttendance();
    } catch (e) {
      setState(() => _isLoading = false);
  ...
  Widget _buildHeader() {
    ...
    bool canCheckOut =
        todaysRecord != null && todaysRecord['clockOutTime'] == null;
    bool isCompleted =
        todaysRecord != null && todaysRecord['clockOutTime'] != null;
  ...
  Widget _buildAttendanceCard(dynamic record) {
    ...
    final bool isAdmin = ApiService.userRole?.toUpperCase() == 'ADMIN';
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // show employee info if admin
                          if (isAdmin)
                            Text(
                              'ID: ${record['employeeId']}${record['employeeName'] != null ? ' - ${record['employeeName']}' : ''}',
                              style: const TextStyle(
                                fontWeight: FontWeight.bold,
                                fontSize: 14,
                              ),
